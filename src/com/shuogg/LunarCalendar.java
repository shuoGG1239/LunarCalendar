...
public class LunarCalendar implements Serializable {
     ...
     */
    private final static int[][] LUNAR_INFO = {{8, 131, 301, 331, 429, 528, 627, 726, 825, 924, 1023, 1122, 1222, 1320}, // 1900
     ...
     * 10天干
     */
    private static final char[] LUNAR_GAN = {'甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'};
    /**
     * 12地支
     */
    private static final char[] LUNAR_ZHI = {'子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'};
     ...
     * 农历年份名
     */
    private static final char[] LUNAR_YEAR_NAME = {'〇', '一', '二', '三', '四', '五', '六', '七', '八', '九'};

    /**
     * 农历月份名
     */
    private static final String[] LUNAR_MONTH_NAME = {"正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二"};

    /**
     * 农历日期名
     */
    private static final String[] LUNAR_DAY_NAME = {"初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十", "十一",
    ...
    private boolean computeBySolarDate(int solarYear, int solarMonth, int solarDate) {
        ...
        if (newMonth == 0) {// 在上一年
            ...
            newMonth = 1; // 农历肯定是1月
            // 取下一年的公历日期码
            int[] nextSolarCodes = LUNAR_INFO[solarYear + 1 - MIN_YEAR];
            // 取下一年农历1月1号公历日期码
    ...
    public String getDayName(int lunarDay) {
        return LUNAR_DAY_NAME[lunarDay - 1];
    }
    ...
    public String getMonthName(int lunarMonth) {
        return LUNAR_MONTH_NAME[lunarMonth - 1];
    }
    ...
    public String getYearName(int lunarYear) {
        return String.valueOf(LUNAR_YEAR_NAME[lunarYear / 1000]) +
                LUNAR_YEAR_NAME[lunarYear % 1000 / 100] +
                LUNAR_YEAR_NAME[lunarYear % 100 / 10] +
                LUNAR_YEAR_NAME[lunarYear % 10];
    }
    ...
    public static long solarDiff(Calendar solar1, Calendar solar2, int field) {
        ...
        switch (field) {
            case Calendar.SECOND:
                return (long) Math.rint((double) (t1 - t2) / 1000d);
            case Calendar.MINUTE:
                return (long) Math.rint((double) (t1 - t2) / (double) (60 * 1000));
            case Calendar.HOUR:
                return (long) Math.rint((double) (t1 - t2) / (double) (3600 * 1000));
            case Calendar.DATE:
                return (long) Math.rint((double) (t1 - t2) / (double) (24 * 3600 * 1000));
            case Calendar.MONTH:
                return (long) Math.rint((double) (t1 - t2) / (double) (30 * 24 * 3600 * 1000));
            case Calendar.YEAR:
                return (long) Math.rint((double) (t1 - t2) / (double) (365 * 24 * 3600 * 1000));
            default:
                return -1;
        }
    }
    ...
    public static String getTraditionalYearName(int y) {
        y = y - MIN_YEAR + 36;
        return ("" + LUNAR_GAN[y % 10] + LUNAR_ZHI[y % 12] + "年");
    }
